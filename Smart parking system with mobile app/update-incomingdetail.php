...
if (strlen($_SESSION['vpmsaid'] == 0)) {
	...
	if (isset($_POST['submit-in'])) {
		...
		$query = mysqli_query($con, "UPDATE vehicle_info set Status='$status' where ID='$cid'");
		if ($query) {
			$up = mysqli_query($con, "UPDATE `pslot` SET `status`='1' WHERE PLot = '$pknu'");
			if ($up) {
