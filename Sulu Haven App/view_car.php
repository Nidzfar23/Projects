<?php
if (isset($_GET['id'])) {
    $packages = $conn->query("SELECT * FROM `rent` where md5(id) = '{$_GET['id']}'");
    ...
    if (is_dir(base_app . 'uploads/car_' . $id)) {
        $ofile = scandir(base_app . 'uploads/car_' . $id);
        foreach ($ofile as $img) {
            if (in_array($img, array('.', '..')))
                continue;
            $files[] = validate_image('uploads/car_' . $id . '/' . $img);
        }
    }
}
?>
                    ...
                    <div class="w-100 d-flex justify-content-between">
                        <span
                            class="rounded-0 btn-flat btn-sm btn-primary d-flex align-items-center  justify-content-between"><i
                                class="fa fa-tag"></i> <span class="ml-1">
                                <?php echo number_format($cost) ?>
                            </span></span>
                        <button class="btn btn-flat btn-warning" type="button" id="book">Rent Now</button>
                    </div>
                </div>
            </div>
            <div class="col-md-7">
                ...
                <?php foreach ($feed as $r): ?>
                    <div class="w-100 d-flex justify-content-between  align-items-center">
                        ...
                        <span class='text-muted'>
                            <?php echo date("Y-m-d H:i A", strtotime($r['date_created'])) ?>
                        </span>
                    </div>
    ...
    $(function () {
        $('#book').click(function () {
            if ("<?php echo $_settings->userdata('id') ?>" > 0)
                uni_modal("Book a Car", "book_car.php?car_id=<?php echo $id ?>");
            else
