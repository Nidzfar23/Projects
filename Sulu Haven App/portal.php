...
<header class="masthead">
	<div class="container">
		<div class="masthead-subheading">Welcome To Sulu Haven Mobile App</div>
		<div class="masthead-heading text-uppercase">Discover the captivating tourist spots in Sulu</div>
		<a class="btn btn-primary btn-xl text-uppercase" href="#home">View Tours</a>
	</div>
</header>
<!-- Services-->
<section class="page-section" id="home" style="background-color: #d1fbff">
	<div class="container">
		<h2 class="text-center">Travel Destination</h2>
						...
						<div class="card-body">
							...
							<div class="w-100 d-flex justify-content-end">
								<a href="./?page=view_package&id=<?php echo md5($row['id']) ?>"
									class="btn btn-sm btn-flat btn-warning">Destination Spot <i
		...
		<div class="d-flex w-100 justify-content-end">
			<a href="./?page=packages" class="btn btn-flat btn-warning mr-4">Explore Landmarks <i
					class="fa fa-arrow-right"></i></a>
		</div>
	</div>
</section>
...
<section class="page-section" id="home" style="background-color: #d1fbff">
	<div class="container">
		<h2 class="text-center">Rent Car</h2>
		<div class="d-flex w-100 justify-content-center">
			<hr class="border-warning" style="border:3px solid" width="15%">
		</div>
		<div class="row">
			<?php
			$packages = $conn->query("SELECT * FROM `rent` order by rand() limit 3");
			while ($row = $packages->fetch_assoc()):
				$cover = '';
				if (is_dir(base_app . 'uploads/car_' . $row['id'])) {
					$img = scandir(base_app . 'uploads/car_' . $row['id']);
					$k = array_search('.', $img);
					if ($k !== false)
						unset($img[$k]);
					$k = array_search('..', $img);
					if ($k !== false)
						unset($img[$k]);
					$cover = isset($img[2]) ? 'uploads/car_' . $row['id'] . '/' . $img[2] : "";
				}
				$row['description'] = strip_tags(stripslashes(html_entity_decode($row['description'])));
				...
				<div class="col-md-4 p-4 ">
					<div class="card w-100 rounded-0">
						...
						<div class="card-body">
							...
							<div class="w-100 d-flex justify-content-end">
								<a href="./?page=view_car&id=<?php echo md5($row['id']) ?>"
									class="btn btn-sm btn-flat btn-warning">Car Details <i
										class="fa fa-arrow-right"></i></a>
							</div>
						</div>
					</div>
		...
		<div class="d-flex w-100 justify-content-end">
			<a href="./?page=car" class="btn btn-flat btn-warning mr-4">View Cars <i class="fa fa-arrow-right"></i></a>
		</div>
	</div>
</section>
<!-- Accommodation -->
<section class="page-section" id="home">
	<div class="container">
		...
		<div class="row">
			<?php
			$packages = $conn->query("SELECT * FROM `accom` order by rand() limit 3");
			while ($row = $packages->fetch_assoc()):
				$cover = '';
				if (is_dir(base_app . 'uploads/accom_' . $row['id'])) {
					$img = scandir(base_app . 'uploads/accom_' . $row['id']);
					$k = array_search('.', $img);
					if ($k !== false)
						unset($img[$k]);
					$k = array_search('..', $img);
					if ($k !== false)
						unset($img[$k]);
					$cover = isset($img[2]) ? 'uploads/accom_' . $row['id'] . '/' . $img[2] : "";
				}
				$row['description'] = strip_tags(stripslashes(html_entity_decode($row['description'])));
				...
				<div class="col-md-4 p-4 ">
					<div class="card w-100 rounded-0">
						...
						<div class="card-body">
							...
							<div class="w-100 d-flex justify-content-end">
								<a href="./?page=view_accom&id=<?php echo md5($row['id']) ?>"
									class="btn btn-sm btn-flat btn-warning">Accommodation Details <i
										class="fa fa-arrow-right"></i></a>
							</div>
						</div>
					</div>
		...
		<div class="d-flex w-100 justify-content-end">
			<a href="./?page=accom" class="btn btn-flat btn-warning mr-4">View Accommodations <i
					class="fa fa-arrow-right"></i></a>
		</div>
	</div>
</section>
...
<section class="page-section" id="home" style="background-color: #d1fbff">
	<div class="container">
		<h2 class="text-center">Tour Guides</h2>
		<div class="d-flex w-100 justify-content-center">
			<hr class="border-warning" style="border:3px solid" width="15%">
		</div>
		<div class="row">
			<?php
			$packages = $conn->query("SELECT * FROM `guide` order by rand() limit 3");
			while ($row = $packages->fetch_assoc()):
				$cover = '';
				if (is_dir(base_app . 'uploads/guide_' . $row['id'])) {
					$img = scandir(base_app . 'uploads/guide_' . $row['id']);
					$k = array_search('.', $img);
					if ($k !== false)
						unset($img[$k]);
					$k = array_search('..', $img);
					if ($k !== false)
						unset($img[$k]);
					$cover = isset($img[2]) ? 'uploads/guide_' . $row['id'] . '/' . $img[2] : "";
				}
				$row['description'] = strip_tags(stripslashes(html_entity_decode($row['description'])));
				...
				<div class="col-md-4 p-4 ">
					<div class="card w-100 rounded-0">
						...
						<div class="card-body">
							...
							<div class="w-100 d-flex justify-content-end">
								<a href="./?page=view_guide&id=<?php echo md5($row['id']) ?>"
									class="btn btn-sm btn-flat btn-warning">Tour Guide Details <i
										class="fa fa-arrow-right"></i></a>
							</div>
						</div>
					</div>
		...
		<div class="d-flex w-100 justify-content-end">
			<a href="./?page=guide" class="btn btn-flat btn-warning mr-4">View Tour Guides <i
					class="fa fa-arrow-right"></i></a>
		</div>
	</div>
</section>
<!-- Contact-->
<section class="page-section" id="contact" style="background-color: #008CBF">
	<div class="container">
		<div class="text-center">
			<h2 class="section-heading text-uppercase">Contact Us</h2>
			<h3 class="section-subheading text-white">Send us a message for inquiries.</h3>
		</div>
		<!-- * * * * * * * * * * * * * * *-->
