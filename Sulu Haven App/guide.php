<section class="page-section" id="home" style="background-color: #d1fbff">
    <div class="container">
        <h2 class="text-center">Tour Guides Overview</h2>
        <div class="d-flex w-100 justify-content-center">
            <hr class="border-warning" style="border:3px solid" width="15%">
        </div>
        <div class="w-100">
            <?php
            $packages = $conn->query("SELECT * FROM `guide` order by rand() ");
            while ($row = $packages->fetch_assoc()):
                ...
                <div class="card d-flex w-100 rounded-0 mb-3 package-item">
                    ...
                    <div class="card-body">
                        <h5 class="card-title truncate-1">
                            <?php echo $row['title'] ?>
                        </h5>
                        ...
                        <div class="w-100 d-flex justify-content-between">
                            ...
                            <span class="rounded-0 btn btn-flat btn-sm btn-primary">Tour Guide Fee:
                                <?php echo number_format($row['cost']) ?>
                            </span>
                            <a href="./?page=view_guide&id=<?php echo md5($row['id']) ?>"
                                class="btn btn-sm btn-flat btn-warning">View Details <i class="fa fa-arrow-right"></i></a>
                        </div>
                    </div>
                </div>
            <?php endwhile; ?>
