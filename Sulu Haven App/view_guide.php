<?php
if (isset($_GET['id'])) {
    $packages = $conn->query("SELECT * FROM `guide` where md5(id) = '{$_GET['id']}'");
    ...
    if (is_dir(base_app . 'uploads/guide_' . $id)) {
        $ofile = scandir(base_app . 'uploads/guide_' . $id);
        foreach ($ofile as $img) {
            if (in_array($img, array('.', '..')))
                continue;
            $files[] = validate_image('uploads/guide_' . $id . '/' . $img);
        }
    }
}
?>
    ...
    $(function () {
        $('#book').click(function () {
            if ("<?php echo $_settings->userdata('id') ?>" > 0)
                uni_modal("Book a Tour Guide", "book_guide.php?guide_id=<?php echo $id ?>");
            else
