<?php
if (isset($_GET['id'])) {
    $packages = $conn->query("SELECT * FROM `accom` where md5(id) = '{$_GET['id']}'");
    ...
    if (is_dir(base_app . 'uploads/accom_' . $id)) {
        $ofile = scandir(base_app . 'uploads/accom_' . $id);
        foreach ($ofile as $img) {
            if (in_array($img, array('.', '..')))
                continue;
            $files[] = validate_image('uploads/accom_' . $id . '/' . $img);
        }
    }
}
?>
    ...
    $(function () {
        $('#book').click(function () {
            if ("<?php echo $_settings->userdata('id') ?>" > 0)
                uni_modal("Book an Accommodation", "book_accom.php?accom_id=<?php echo $id ?>");
            else
