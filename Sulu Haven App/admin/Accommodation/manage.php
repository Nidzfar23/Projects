...
if (isset($_GET['id']) && $_GET['id'] > 0) {
    $qry = $conn->query("SELECT * from `accom` where id = '{$_GET['id']}' ");
    if ($qry->num_rows > 0) {
...
<div class="card card-outline card-info">
    <div class="card-header">
        <h3 class="card-title">
            <?php echo isset($id) ? "Update " : "Create New " ?> Accommodation
        </h3>
    </div>
    <div class="card-body">
        <form action="" id="car-form">
            ...
            <div class="form-group">
                <label for="tour_location" class="control-label">Location</label>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#car-form').submit(function (e) {
            ...
            start_loader();
            $.ajax({
                url: _base_url_ + "classes/Master.php?f=save_accom",
                ...
                success: function (resp) {
                    if (typeof resp == 'object' && resp.status == 'success') {
                        location.href = "./?page=Accommodation";
                    } else {
                        alert_toast("An error occured", 'error');
                        end_loader();
                        console.log(resp)
