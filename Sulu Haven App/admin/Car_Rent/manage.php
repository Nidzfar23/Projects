...
if (isset($_GET['id']) && $_GET['id'] > 0) {
    $qry = $conn->query("SELECT * from `rent` where id = '{$_GET['id']}' ");
    if ($qry->num_rows > 0) {
...
<div class="card card-outline card-info">
    <div class="card-header">
        <h3 class="card-title">
            <?php echo isset($id) ? "Update " : "Create New " ?> Rent Car
        </h3>
    </div>
    <div class="card-body">
        <form action="" id="car-form">
            ...
            <div class="form-group">
                <label for="title" class="control-label">Car Name</label>
                <textarea name="title" id="" cols="30" rows="2"
                    class="form-control form no-resize"><?php echo isset($title) ? $title : ''; ?></textarea>
            </div>
            <div class="form-group">
                <label for="tour_location" class="control-label">Car Location</label>
                <textarea name="tour_location" id="" cols="30" rows="2"
                    class="form-control form no-resize"><?php echo isset($tour_location) ? $tour_location : ''; ?></textarea>
            </div>
            <div class="form-group">
                <label for="cost" class="control-label">Price</label>
                <input type="number" step="any" class="form form-control" name="cost"
                    value="<?php echo isset($cost) ? $cost : 0; ?>">
            </div>
            <div class="form-group">
    ...
    <div class="card-footer">
        <button class="btn btn-flat btn-primary" form="car-form">Save</button>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#car-form').submit(function (e) {
            ...
            start_loader();
            $.ajax({
                url: _base_url_ + "classes/Master.php?f=save_car",
