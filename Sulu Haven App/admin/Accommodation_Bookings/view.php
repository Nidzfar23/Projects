...
include '../../config.php';
if (isset($_GET['id'])) {
    $qry = $conn->query("SELECT b.*,p.title,concat(u.firstname,' ',u.lastname) as name FROM book_list b inner join `accom` p on p.id = b.accom_id inner join users u on u.id = b.user_id where b.id = '{$_GET['id']}' ");
    $row = $qry->fetch_assoc();
    foreach ($row as $k => $v) {
        $$k = $v;
    }
