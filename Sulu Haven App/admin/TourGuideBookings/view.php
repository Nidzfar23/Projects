...
include '../../config.php';
if (isset($_GET['id'])) {
    $qry = $conn->query("SELECT b.*,p.title,concat(u.firstname,' ',u.lastname) as name FROM book_list b inner join `guide` p on p.id = b.guide_id inner join users u on u.id = b.user_id where b.id = '{$_GET['id']}' ");
    $row = $qry->fetch_assoc();
    foreach ($row as $k => $v) {
        $$k = $v;
    }
...
<p><b>Details:</b> <span class="truncate">
        <?php echo strip_tags(stripslashes(html_entity_decode($title))) ?>
    </span></p>
<p><b>Schedule:</b>
    <?php $schedule = $row['schedule']; ?>
    <?php echo date("F d, Y", strtotime($schedule)) ?>
</p>
