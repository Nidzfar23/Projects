...
<section class="page-section">
    <div class="container">
        <div class="w-100 justify-content-between d-flex">
            <h4><b>Booked Accommodations</b></h4>
            <a href="./?page=edit_account" class="btn btn btn-primary btn-flat">
            ...
            <tbody>
                ...
                while ($row = $qry->fetch_assoc()):
                    ...
                    <tr>
                        ...
                        <td>
                            <?php echo date("Y-m-d", strtotime($row['schedule'])) ?>
                        </td>
                        ...
                        <td align="center">
                            <button type="button"
                                class="btn btn-flat btn-default border btn-sm dropdown-toggle dropdown-icon"
                            ...
                            <div class="dropdown-menu" role="menu">
                                ...
                                <?php if ($row['status'] == 3 && $review <= 0): ?>
                                    <a class="dropdown-item submit_review" href="javascript:void(0)"
                                        data-id="<?php echo $row['package_id'] ?>">Submit Review</a>
                                <?php endif; ?>
                                <div class="dropdown-divider"></div>
                                <a class="dropdown-item cancel_data" href="javascript:void(0)"
                                    data-id="<?php echo $row['id'] ?>">Cancel</a>
                            </div>
                        </td>
                    </tr>
                <?php endwhile; ?>
    ...
    <div class="container">
        ...
        <table class="table table-stripped text-dark">
            ...
            <tbody>
                <?php
                $i = 1;
                $qry = $conn->query("SELECT b.*,p.title FROM book_list b inner join `rent` p on p.id = b.car_id where b.user_id ='" . $_settings->userdata('id') . "' order by date(b.date_created) desc ");
                while ($row = $qry->fetch_assoc()):
                    $review = $conn->query("SELECT * FROM `rate_review` where package_id='{$row['id']}' and user_id = " . $_settings->userdata('id'))->num_rows;
                    ?>
                    <tr>
