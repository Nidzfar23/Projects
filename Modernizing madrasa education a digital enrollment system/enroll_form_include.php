	...
	<div class="col-md-6">
		...
		<div class="form-group">
			...
			<select name="gender" id="gender" class="custom-select browser-default">
				<option value="male" <?php echo isset($meta['gender']) && $meta['gender'] == 'male' ? 'selected' : '' ?>>Male
				</option>
				<option value="female" <?php echo isset($meta['gender']) && $meta['gender'] == 'female' ? 'selected' : '' ?>>
					Female</option>
			</select>
		</div>
		<div class="form-group">
			<label for="dob" class="control-label">Date of Birth</label>
			<input type="dob" id="dob" name="dob" class="form-control datepicker"
		...
		<div class="form-group">
			...
			<textarea type="text" id="address" name="address" class="form-control"
				required><?php echo isset($meta['address']) ? $meta['address'] : '' ?></textarea>
		</div>
	</div>
...
<div class="col-md-6">
		...
		<select name="level_section_id" id="level_section_id" class="custom-select browser-default select2">
			...
			while ($row = $ls->fetch_assoc()) {
				if (isset($fac_arr[$row['id']])) {
					?>
					<option value="<?php echo $row['id'] ?>" <?php echo isset($meta['level_section_id']) && $meta['level_section_id'] == $row['id'] ? 'selected' : '' ?>>
						<?php echo (ucwords($row['ls'])) . "  (Adviser: " . $fac_arr[$row['id']] . " )" ?>
					</option>
				<?php }
			} ?>
		</select>
	...
	<div class="form-group">
		...
		<input name="sla" id="sla" class="form-control" required="" <?php echo $etype == 2 ? 'readonly' : '' ?>
			value="<?php echo isset($meta['last_school']) ? $meta['last_school'] : (isset($smeta['name']) && $etype == 2 ? $smeta['name'] : '') ?>">
	</div>
	<div class="form-group">
	...
	<!-- <div class="form-group">
		<label for="sla">Status</label>
		<input name="sla" id="sla" class="form-control" required="" <?php echo $etype == 2 ? 'readonly' : '' ?>
			value="<?php echo isset($meta['status']) ? $meta['status'] : (isset($smeta['name']) && $etype == 2 ? $smeta['name'] : '') ?>">
	</div> -->

	<!-- <div class="form-group">
		<label for="sla">Status</label>
		<?php
		$statusValue = isset($meta['status']) ? $meta['status'] : (isset($smeta['name']) && $etype == 2 ? $smeta['name'] : '');
		$statusText = $statusValue == 0 ? 'Not officially enrolled' : 'Officially enrolled';
		?>
		<input name="sla" id="sla" class="form-control" required="" readonly value="<?php echo $statusText; ?>">
	</div> -->
	...
	<div class="form-group">
		...
		$statusText = $statusValue == 0 ? 'Not officially enrolled' : 'Officially enrolled';
		?>
		<select name="stat" id="stat" class="custom-select browser-default" required="">
			<option value="0" <?php echo $statusValue == 0 ? 'selected' : '' ?>>Not officially enrolled</option>
			<option value="1" <?php echo $statusValue == 1 ? 'selected' : '' ?>>Officially enrolled</option>
		</select>
	</div>
