...
class Action
{
	...
	function load_student_off()
	{
		$qry = $this->db->query("SELECT e.*,CONCAT(s.firstname,' ',s.middlename,' ',s.lastname) as name,CONCAT(f.firstname,' ',f.middlename,' ',f.lastname) as fname,CONCAT(ls.level,'-',ls.section) as ls,s.type from enrollment e inner join faculty f on f. id = e.faculty_id inner join level_section ls on e.level_section_id = ls.id inner join student_list s on s.id = e.student_id  where s.status = 1 and e.status = 1 and e.school_year in (SELECT id from school_year where is_on = 1 ) ");
		$data = array();
		while ($row = $qry->fetch_assoc()) {
			if ($row['type'] == 1)
				$row['user_type'] = "New";
